...
func (b *Builder) Create() ([]builder.ResourceApplier, error) {
	return []builder.ResourceApplier{CreateApplier(b.Build())}, nil
}

// PluginsEnricher allows to add extra plugins to the kustomize builder.
type PluginsEnricher struct {
	Plugins []resmap.Transformer
}

func (p *PluginsEnricher) Enrich(resourceBuilder builder.ResourceBuilder) {
	builderValue := reflect.ValueOf(resourceBuilder)
